...
export default function SendMoneyRequest( props) {
  ...
  let Deposit = async () =>{
    ...
    if(remark === '' || remark.length === 0 || user.username === ''){
      setAlert(true);
      setAlertMsg('Please Enter some review regarding responce');
      return;
    }
  ...
  if(!API){
    ...
    let config = {
      method: 'post',
      url: configure.SERVER_URL+'users/sendmoneyResponce',
      headers: { 
    ...
    .then((res) => {
      console.log(res.data)
                ...
                setTimeout(() => {
                  document.getElementById('SendMoneyRequest').click();
                }, 2000);
            })
            .catch((error) => {
                console.log(error);
  ...
  return (
    <div className="modal" id="SendMoneyRequest">
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h6 className="modal-title text-dark" style={{display:'flex'}}>Send Money Response &nbsp;<p className='text-success'>{props.data}</p></h6>
            <button
              type="button"
              className="btn-close"
